...
class _DoctorHomeScreenState extends State<DoctorHomeScreen> {
  ...
  void initState() {
    super.initState();
    // Charger les notifications de démo pour les médecins
          ...
          child: SafeArea(
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  _NavItem(icon: Icons.dashboard_outlined, activeIcon: Icons.dashboard_rounded, label: 'Tableau', index: 0, current: _currentIndex, onTap: () => setState(() => _currentIndex = 0)),
                  _NavItem(icon: Icons.calendar_today_outlined, activeIcon: Icons.calendar_today_rounded, label: 'Agenda', index: 1, current: _currentIndex, badge: doctor.pendingAppointments.length, onTap: () => setState(() => _currentIndex = 1)),
                  _NavItemMessages(index: 2, current: _currentIndex, onTap: () => setState(() => _currentIndex = 2)),
                  _NavItem(icon: Icons.people_outline, activeIcon: Icons.people_rounded, label: 'Demandes', index: 3, current: _currentIndex, badge: pendingTreatingCount, onTap: () => setState(() => _currentIndex = 3)),
                  _NavItem(icon: Icons.person_outline_rounded, activeIcon: Icons.person_rounded, label: 'Profil', index: 4, current: _currentIndex, onTap: () => setState(() => _currentIndex = 4)),
                ],
              ),
            ),
          ),
              ...
              child: Column(
                children: [
                  ProfilePhotoPickerSquare(
                    existingBase64: auth.currentUserAvatarBase64,
                    onPhotoSelected: (bytes, b64) async {
                      if (b64.isEmpty) return;
                      await auth.updateAvatarBase64(b64);
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Photo professionnelle mise à jour !'),
                            backgroundColor: Color(0xFF1A7A5A),
                  ...
                  Text(auth.userName, style: const TextStyle(fontFamily: 'Poppins', fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.textWhite)),
                  Text(auth.doctorProfile?.specialty ?? '', style: TextStyle(fontFamily: 'Poppins', fontSize: 13, color: AppColors.textWhite.withValues(alpha: 0.8))),
...
class _ProfStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(value, style: const TextStyle(fontFamily: 'Poppins', fontSize: 22, fontWeight: FontWeight.bold, color: AppColors.textWhite)),
        Text(label, style: TextStyle(fontFamily: 'Poppins', fontSize: 12, color: AppColors.textWhite.withValues(alpha: 0.8))),
      ],
    );
  }
}
